    ...
    const fetchContactInfo = async () => {
      try {
        const response = await fetch(
          "http://localhost:1339/api/contact-info?populate=deep,10"
        );
        ...
        const imageUrl = `
http://localhost:1339${data.data.attributes.contact.image.data.attributes.url}`;
        setImageData({
          url: imageUrl,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch("http://localhost:1339/api/contacts", {
        method: "POST",
        headers: {
